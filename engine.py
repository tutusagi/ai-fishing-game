...
# 选择性详细播报（v2.2.1）：传说/神话=完整演出；新发现=🆕(名·稀有·尺寸·价值 + 图鉴描述 + 首次收录奖励)；
# 稀有/史诗=一行 + 描写；普通/少见=极简一行。去掉 [实例id]（要卖看 inventory）、去掉重复的「图鉴新发现」标签；🎉 留给图鉴里程碑。
def _format_catch(f, size, value, inst, first):
    u = f["size_unit"]; r = f["rarity"]; rl = RARITY[r]["label"]
    cf = ("\n🫧 " + f["capture_feel"]) if f.get("capture_feel") else ""   # 手感（若有），与图鉴描述并存
    flavor = f.get("description", "") + cf
    if r in ("legendary", "mythic"):   # 完整演出（👑/❖ 已够隆重，不再加 🎉）
...
def _cast_step(rng, bait_id):
    ...
    _bite = _bite_line(rng, f["rarity"])   # 仍抽一次保持随机流（同 seed 同结果），但 v2.2.1 起不再显示咬钩过场，去重省 token
    ...
    # 首次收录奖励已并入 _format_catch 标题；🎉 改由 _milestone_line 在达成图鉴里程碑时才播；footer 去掉（cmd 末尾统一附 📊 状态栏）
    return {"text": season_msg + "%s%s%s%s%s%s" % (_format_catch(f, size, value, inst, first), _milestone_line(f, first), fever_line, luck_seg, _ambience(loc, rng), _secret_hint()),
            "consumed": True, "kind": "fish", "fish_name": f["name"], "rarity": f["rarity"], "first": first, "season_changed": season_changed, "luck": luck_id, "fever_hit": fever_line != "", "pref": pref}
...
def _cast_many(bait_id, times, stop_on):
    ...
    stop_reason = None   # 只在提前停（停因/没饵）时才说一句，钓满不赘述
